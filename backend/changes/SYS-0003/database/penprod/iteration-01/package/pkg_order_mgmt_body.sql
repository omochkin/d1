...
-->>> BEGIN CHANGE: bugfix/SYS-0001
    ...
    PROCEDURE CANCEL_ORDER(p_order_id IN NUMBER, p_reason IN VARCHAR2,
        p_cancelled_by IN VARCHAR2, p_status OUT VARCHAR2, p_message OUT VARCHAR2
    ) AS v_current_status VARCHAR2(100);
    BEGIN
        BEGIN
            SELECT NVL(STATUS,'UNKNOWN') INTO v_current_status
            FROM ORDERS WHERE ORDER_ID=p_order_id;
        EXCEPTION WHEN NO_DATA_FOUND THEN
            p_status:='NOT_FOUND'; p_message:='Order '||p_order_id||' does not exist'; RETURN;
        END;
        IF v_current_status IN ('SHIPPED','DELIVERED') THEN
            p_status:='ERROR';
            p_message:='Cannot cancel order '||p_order_id||' -- status: '||v_current_status;
            RETURN;
        END IF;
        IF v_current_status='CANCELLED' THEN
            p_status:='ERROR'; p_message:='Order '||p_order_id|| ' is already cancelled'; RETURN;
        END IF;
        UPDATE ORDERS SET STATUS='CANCELLED',LAST_UPDATED_DATE=SYSDATE WHERE ORDER_ID=p_order_id;
        COMMIT;
        p_status:='SUCCESS';
        p_message:='Order '||p_order_id||' cancelled by '||p_cancelled_by||'. Reason: '||p_reason;
    EXCEPTION WHEN OTHERS THEN ROLLBACK;
        p_status:='ERROR'; p_message:='Error: '||SQLERRM;
    END CANCEL_ORDER;
